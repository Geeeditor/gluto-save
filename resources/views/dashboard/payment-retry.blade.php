    ...
    <div class="relative py-3 signup">
        ...
        <div class="bg-gray-100 shadow-gray-300 shadow-lg mx-auto my-2 welcome-modal">
            ...
            <div class="mx-auto mt-3 px-5 md:px-7 py-5 rounded-lg investment-details-wrapper">
                ...
                <form class="d-flex flex-col gap-5 space-mobile" action="{{route('dashboard.payments.update', ['id' => $transaction->id])}}" method="POST"
                    enctype="multipart/form-data">
                    @php
                        $isDisabled = $transaction->payment_type == 'registration' || $transaction->payment_type == 'subscription' ? true : false;
                        //
                    ...
                    <div class="form-inline align-items-center row">
                        ...
                        <div class="col-sm-8">
                            <input type="number" {{ $isDisabled ? 'readonly' : '' }} value="{{ $transaction->amount }}" required class="form-control input"
                                name="amount">
                            {{-- {{ $isDisabled ? 'disabled' : '' }} --}}
                        </div>
                    </div>

                    <input type="text" name="transaction_reference" value="{{ $transaction->transaction_reference }}" hidden>
                    <input type="text" name="payment_id" value="{{ $transaction->payment_id }}" hidden>
                    ...
                    <div id="payment_upload_container" class="hidden top-0 left-0 fixed justify-center shadow-lg blur-bg rounded-lg w-full h-[100vh] md:overflow-hidden overflow-y-scroll">
